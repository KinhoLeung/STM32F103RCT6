...
//   b = lcd_rd_data();
//   g = r & 0XFF; /* 对于 9341/5310/5510/7789/9806,
//                    第一次读取的是RG的值,R在前,G在后,各占8位 */
//   g <<= 8;
//   return (((r >> 11) << 11) | ((g >> 10) << 5) |
//           (b >> 11)); /* 9341/5310/5510/7789/9806 需要公式转换一下 */
// }
 ...
 * @retval      无
 */
void lcd_display_on(void) { lcd_wr_regno(0X29); /* 开启显示 */ }
 ...
 * @retval      无
 */
void lcd_display_off(void) { lcd_wr_regno(0X28); /* 关闭显示 */ }
...
void lcd_init(void) {
  ...
  __HAL_AFIO_REMAP_SWJ_NOJTAG(); /* 禁止JTAG, 使能SWD,
                                    释放PB3,PB4两个引脚做普通IO用 */
  ...
  HAL_GPIO_Init(LCD_BL_GPIO_PORT,
                &gpio_init_struct); /* LCD_BL引脚模式设置(推挽输出) */
  ...
  HAL_GPIO_Init(LCD_DATA_GPIO_PORT,
                &gpio_init_struct); /* LCD_DATA引脚模式设置 */
  LCD_WR(1);                        /* WR 默认高电平 */
  LCD_RD(1);                        /* RD 默认高电平 */
  LCD_CS(1);                        /* CS 默认高电平 */
  LCD_RS(1);                        /* RS 默认高电平 */
...
//  * @retval      无
//  */
// void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t
// size, uint16_t color)
// {
//     uint8_t t, temp;
//     uint8_t enshow = 0;
...
//  * @retval      无
//  */
// void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t
// size, uint8_t mode, uint16_t color)
// {
//     uint8_t t, temp;
//     uint8_t enshow = 0;
...
//                 {
//                     lcd_show_char(x + (size / 2) * t, y, '0', size, mode &
//                     0X01, color); /* 用0占位 */
//                 }
//                 else
//                 {
//                     lcd_show_char(x + (size / 2) * t, y, ' ', size, mode &
//                     0X01, color); /* 用空格占位 */
//                 }
